...
public:
	...
	public:
		int dimensions;
		T initial_condition; // IC @ x=0
		vector<T> x, y; // Containers for x and y
		T(*differential_equation)(T, T);
	}Eqn;

protected:
	T step_size; // Step Size Used
	T target_x; // x Value of Interest
	T initial_condition;
};
...
T RK38<T>::Solve()
{
	...
	for (float i = 1; this->Eqn.x.back()<this->target_x; ++i)
	{
		...
		k2 = this->step_size*this->Eqn.differential_equation(this->Eqn.x.at(i - 1) + (this->step_size / 3.0), this->Eqn.y.at(i - 1) + (k1 / 3.0));
		k3 = this->step_size*this->Eqn.differential_equation(this->Eqn.x.at(i - 1) + (2.0 * this->step_size) / 3.0, this->Eqn.y.at(i - 1) - (k1 / 3.0) + k2);
